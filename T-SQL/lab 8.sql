...
SELECT COUNT(*) FROM EMPLOYEE AS NO_OF_EMPLOYEE
...
SELECT CITY,MAX(SALARY) AS MAX,MIN(SALARY)AS MIN
FROM EMPLOYEE
WHERE GENDER='FEMALE'
GROUP BY CITY

--27. Calculate department, city, and gender wise average salary. 
SELECT DEPARTMENT,CITY,GENDER,AVG(SALARY) AS AVG
FROM EMPLOYEE
...
SELECT MAX(SALARY) AS MAX,MIN(SALARY) AS MIN,MAX(SALARY)-MIN(SALARY) AS DIFFERENCE
FROM EMPLOYEE
...
SELECT DEPARTMENT,CITY,MAX(SALARY) AS MAX
FROM EMPLOYEE
WHERE CITY LIKE 'R%'
GROUP BY DEPARTMENT,CITY